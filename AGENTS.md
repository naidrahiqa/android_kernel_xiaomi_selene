...
- **Root solution:** ReSukiSU (`ReSukiSU/ReSukiSU`, fork SukiSU-Ultra, main @ `7bb6f0df` = v4.2.0-rc1 + 32 commits, **KSU_VERSION 35093**).
  ...
  - Kbuild di-patch lokal: fallback version pin tanpa `.git` (`KSU_LOCAL_VERSION := 4393`, tag `v4.2.0-rc1`, sha `7bb6f0df`, branch `main`). `CONFIG_KPROBES` tidak dibutuhkan; `CONFIG_EXT4_FS=y` dipertahankan.
...
- ReSukiSU: `ReSukiSU/ReSukiSU` main @ `7bb6f0df` (local copy di `resukisu/kernel/`).
...
- ReSukiSU: ReSukiSU main @ `7bb6f0df` via `drivers/kernelsu` symlink (manual hook, non-GKI)
...
- Source: `resukisu/kernel/` (direct copy, not submodule). Current: main @ `7bb6f0df` (v4.2.0-rc1 + 32 commits, 4393 commits, **KSU_VERSION 35093**).
...
- **Versi di-pin via fallback di `Kbuild`** (patch lokal — upstream `$(error ...)` kalau bukan git submodule): `KSU_LOCAL_VERSION := 4393`, `KSU_TAG_NAME := v4.2.0-rc1`, `KSU_COMMIT_SHA := 7bb6f0df`, `KSU_BRANCH_NAME := main`. Formula `KSU_VERSION = 30000 + commits + 700` → 35093. Jangan set ke 1 (manager tidak deteksi root).
