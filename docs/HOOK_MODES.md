...
`03b60f26`, KSU_VERSION 35090) di Phrolova Kernel (Linux 4.14.356, non-GKI,
MT6768 selene).
...
- **Manual hook (sejak v0.9.0):** ReSukiSU main @ `03b60f26` (v4.2.0-rc1 + 10
  commits, KSU_VERSION 35090) menggantikan KernelSU-Next hookless
...
- Kbuild di-patch lokal: fallback version pin tanpa `.git`
  (`KSU_LOCAL_VERSION := 4390` → `KSU_VERSION = 30000 + 4390 + 700 = 35090`).
- Manager: multi-manager (`CONFIG_KSU_MULTI_MANAGER_SUPPORT=y`). Rekomendasi
  ReSukiSU manager — match KSU_VERSION 35090:
...
- Diagnosa cepat saat boot:
  ...
  # "-- ReSukiSU version code: 35090" → versi benar
...
| v0.9.0+ | ReSukiSU main @ 03b60f26 | Manual hook (fs/exec.c, fs/open.c, fs/stat.c, kernel/reboot.c) |
