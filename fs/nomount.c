...
#else
    err = notify_change(info->r_path.dentry, attr, NULL);
#endif
...
#else
        return __vfs_setxattr(info->r_path.dentry, d_inode(info->r_path.dentry), xattr_full_name(handler, name), buffer, size, flags);
#endif
    }
